// ==UserScript==
// @name         Torn Vault Request - Backend Only
// @namespace    TornVaultRequestBackendOnly
// @version      3.0.0
// @description  Backend-only Torn vault request helper. Members use an automatic backend URL; webhook URLs stay server-side.
...
(() => {
  ...
  */
  ...
  const DEFAULT_FACTION_WEBHOOK_URL = 'https://gooey-eagle-rentable.ngrok-free.dev';
  ...
  async function saveBackendWebhookSettingsFromPanel() {
    const adminKeyInput = cleanText($('backendAdminKey')?.value || '');
    settings.backendAdminKey = adminKeyInput === '********' ? settings.backendAdminKey : adminKeyInput;
    ...
    if (!settings.backendAdminKey) throw new Error('Paste the ADMIN_KEY from your backend .env first.');
    ...
    await backendRequest('/api/admin/settings', {
      adminKey: settings.backendAdminKey,
      requestWebhookUrl,
      userNotifyWebhookUrl
    });
    ...
    showToast('Webhook URLs saved to backend. They were not saved in the userscript.', 'ok');
  }
  ...
  async function fulfillRequestViaBackend(record, banker) {
    if (!settings.backendAdminKey) throw new Error('Add the backend admin key in Settings first.');
...
Backend-only mode: this panel sends requests to the automatic backend URL built into this userscript. Members do not need Discord webhook URLs.
  ...
  function openSettingsGate() {
    ...
    panel.innerHTML = `
      ...
      <div class="${APP}-card">
        ...
        <p class="${APP}-note">
          Settings control backend webhook setup, embed messages, and banker balance checks.
      ...
      <div class="${APP}-card">
        ...
        <p class="${APP}-note">
          ...
          It is used to check faction access and banker-side vault balance data.
        </p>
      </div>
    `;
  ...
  async function sendTestEmbed() {
    ...
    if (!settings.backendAdminKey) {
      showToast('Paste the ADMIN_KEY from your backend .env first.', 'warn');
      return;
    }
  ...
  function openSettingsPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="${APP}-card">
        ...
        <label for="${APP}-backendAdminKey">Backend admin key</label>
        <input id="${APP}-backendAdminKey" type="password" placeholder="Backend ADMIN_KEY" autocomplete="off" value="${settings.backendAdminKey ? '********' : ''}" />
        <p class="${APP}-note">
          Banker/leader only. This token lets the script save webhook URLs to the backend and mark requests fulfilled/canceled.
        </p>
        ...
        <p class="${APP}-note">
          Webhook URLs are sent to the backend and are not saved in member scripts. Members only need the script with the automatic backend URL.
        </p>
      </div>
    ...
    for (const id of ['backendAdminKey', 'webhookUrl', 'userNotifyWebhookUrl', 'embedTitle', 'embedDescription', 'embedFooter', 'embedColorPicker', 'timeoutNotifyTitle', 'timeoutNotifyMessage', 'timeoutNotifyFooter', 'timeoutNotifyColor', 'fulfilledNotifyTitle', 'fulfilledNotifyMessage', 'fulfilledNotifyFooter', 'fulfilledNotifyColor', 'previewUser', 'previewAmount']) {
      const el = $(id);
      if (!el) continue;
    ...
    $('saveSettings').addEventListener('click', () => {
      ...
      updateEmbedPreview();
      updateNoticePreview();
      showToast('Local admin/message settings saved.', 'ok');
    });
