...
// @version      3.3.1
// @description  Backend-only Torn vault request helper with inclusive tiered faction-wide Xanax licences. Active licences cover new faction members; next extension uses current member count.
  ...
  */
  ...
  async function buyExtendFactionLicenceFromPanel() {
    ...
    if (paymentBox) {
      ...
      const tier = p.licenceTier || data.licence?.licenceTier || {};
      paymentBox.innerHTML = `
        <b>Faction licence extended.</b><br>
        Members: ${escapeHtml(String(p.memberCount ?? data.licence?.memberCount ?? 0))}<br>
        Current extend rate: 1 Xanax = ${escapeHtml(String(p.daysPerXanax || data.licence?.daysPerXanax || '?'))} day(s) ${tier.label ? `(${escapeHtml(tier.label)})` : ''}<br>
        Added ${escapeHtml(String(p.addedDays || data.licence?.lastAddedDays || 0))} day(s).<br>
        New members can use this faction licence while it is active. Next extension uses the then-current member count.<br>
        ${escapeHtml(msg)}
      `;
    }

    showToast(`Faction licence extended by ${p.addedDays || data.licence?.lastAddedDays || 0} days.`, 'ok');
  ...
  function openRequestPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="${APP}-card">
        ...
        <p class="${APP}-note">
          Buy/extend is for members with <b>faction API permission</b>. Price depends on current faction member count only when extending: 0-25 = 15 days/Xanax, 26-50 = 12, 51-75 = 9, 76-100 = 6. New members can use an already-active faction licence.
        </p>
        ...
        <div class="${APP}-row">
          <button type="button" class="${APP}-btn good" id="${APP}-buyExtendLicence">Buy / Extend Faction Licence</button>
        </div>

        <div id="${APP}-licencePaymentStatus" class="${APP}-preview warn">Buy/extend calculates days from current faction member count.</div>
      </div>
...
Backend-only mode: this panel sends requests to the automatic backend URL built into this userscript. Members do not need Discord webhook URLs.
    ...
    $('buyExtendLicence').addEventListener('click', async () => {
      ...
      try {
        ...
        if (liveBtn) {
          liveBtn.disabled = false;
          liveBtn.textContent = 'Buy / Extend Faction Licence';
        }
      }
    });
