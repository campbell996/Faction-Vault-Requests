...
// @connect      fvr-backend-yourname.onrender.com
// @connect      localhost
// @connect      127.0.0.1
// ==/UserScript==
...
(() => {
  ...
  */
  ...
  const DEFAULT_SETTINGS = {
    ...
    embedFooter: 'Backend-only. Manual banker approval required.',
    embedColor: 'e67300',
    embedPreset: 'default',
  ...
  const PRESETS = {
    default: {
      ...
      footer: 'Backend-only. Manual banker approval required.',
      color: 'e67300'
    },
    rwph: {
      name: 'RWPH Style',
  ...
  if (!settings.backendUrl) settings.backendUrl = DEFAULT_BACKEND_URL;
  ...
  function cleanBackendUrl(value = settings.backendUrl || DEFAULT_BACKEND_URL) {
    return String(value || '').trim().replace(/\/+$/, '');
  }

  function backendLooksConfigured(value = settings.backendUrl || DEFAULT_BACKEND_URL) {
    const url = cleanBackendUrl(value);
    return /^https?:\/\/.+/i.test(url) && !/REPLACE-WITH-YOUR-BACKEND-URL/i.test(url);
  }

  function getActiveBackendUrl(value = settings.backendUrl || DEFAULT_BACKEND_URL) {
    return cleanBackendUrl(value || DEFAULT_BACKEND_URL);
  }
  ...
  async function backendRequest(path, body = null, options = {}) {
    if (!backendLooksConfigured()) {
      throw new Error('Backend URL is not configured. Replace DEFAULT_BACKEND_URL in the userscript with your deployed backend URL.');
    }
    ...
    const res = await gmRequest(req);

    let data = {};
    try {
      data = JSON.parse(res.responseText || '{}');
    } catch {
      data = { ok: false, error: res.responseText || 'Invalid backend response.' };
    }
  ...
  function getCurrentPageRequestParams() {
    return {
      ...
      backendUrl: readFvrParam('fvrBackendUrl'),
  ...
  function buildRequesterStatusSyncUrl(record, status, banker = {}, reason = '') {
    ...
    const params = new URLSearchParams({
      ...
      fvrReason: reason || '',
      fvrBackendUrl: record.backendUrl || getActiveBackendUrl(),
        ...
        showToast(
          synced
            ...
            : removedCount
              ...
              : 'Pending requests and notifications refreshed from the backend.',
          'ok'
        );
      }
      ...
      embeds: [
        {
          ...
          fields,
          footer: { text: footer || 'Backend-only. Manual banker approval required.' },
  ...
  async function expireRequest(record) {
    ...
    try {
      const backendStatus = await getBackendRequestStatus(record);
      const status = String(backendStatus?.status || '').toLowerCase();

      if (status === 'expired' || status === 'timedout' || status === 'timed_out') {
        markRequestAction(record.id, 'expired', { backendExpired: true });
        ...
        showToast('Vault request timed out and was removed from pending requests.', 'warn');
      }
    } catch (err) {
      console.warn('[Vault Request] Backend timeout check failed:', err);
    }
  }
  ...
  async function checkPendingRequestFinalStatuses() {
    ...
    for (const req of pending) {
      ...
      if (!finalStatus) {
        try {
          checkedBackend += 1;
          const backendStatus = await getBackendRequestStatus(req);
          const status = String(backendStatus?.status || '').toLowerCase();

          if (status === 'fulfilled') finalStatus = 'fulfilled';
          if (status === 'cancelled' || status === 'canceled') finalStatus = 'cancelled';
          if (status === 'expired' || status === 'timedout' || status === 'timed_out') finalStatus = 'timedout';
        } catch (err) {
          errors += 1;
          console.warn('[Vault Request] Could not check backend request status:', err);
        }
      }
    ...
    return { removed, checkedBackend, errors };
  }

  async function checkPendingRequestTimeouts() {
    await checkPendingRequestFinalStatuses();
    updateRequestNotificationsPanel();
  }
  ...
  function licenceStatusText(info = settings.licenceInfo) {
    if (!info) return 'Licence not checked yet.';
    if (info.active) return `Faction licence active. ${info.daysRemaining || 0} day${Number(info.daysRemaining || 0) === 1 ? '' : 's'} remaining.`;
    return 'Faction licence missing or expired.';
  }

  function updateLicencePanel(data = null) {
    if (data?.faction) {
      settings.factionId = String(data.faction.factionId || '');
      settings.factionName = data.faction.factionName || '';
    }

    if (data?.licence) {
      settings.licenceInfo = data.licence;
    }

    const status = $('licenceStatus');
    if (status) {
      const factionLabel = settings.factionId
        ? `${settings.factionName || 'Faction'} [${settings.factionId}]`
        : 'Unknown faction';
      const info = settings.licenceInfo;
      const active = !!info?.active;

      status.className = `${APP}-preview ${active ? 'ok' : 'warn'}`;
      const memberLine = info?.memberCount !== undefined ? `<br><span class="${APP}-tiny">Members: ${escapeHtml(String(info.memberCount || 0))}</span>` : '';
      const tierLine = info?.daysPerXanax ? `<br><span class="${APP}-tiny">Next extend rate: 1 Xanax = ${escapeHtml(String(info.daysPerXanax))} day(s)</span>` : '';

      status.innerHTML = `
        <b>${escapeHtml(factionLabel)}</b><br>
        ${escapeHtml(licenceStatusText(info))}
        ${memberLine}
        ${tierLine}
        ${info?.expiresAt ? `<br><span class="${APP}-tiny">Expires: ${escapeHtml(new Date(Number(info.expiresAt)).toLocaleString())}</span>` : ''}
      `;
    }

    if ($('licenceFactionId')) $('licenceFactionId').value = settings.factionId || '';
    saveSettings();
  }

  function getSavedTornApiKeyFromLicencePanel() {
    const inputValue = cleanText($('limitedApiKey')?.value || '');
    if (inputValue && inputValue !== '********') return inputValue;
    return cleanText(settings.apiKey || '');
  }

  async function saveAndCheckLimitedApiKey(showToastOnSuccess = true) {
    const key = getSavedTornApiKeyFromLicencePanel();

    if (!key || key.length < 8) {
      throw new Error('Paste a limited Torn API key first.');
    }
    ...
    const data = await backendRequest('/api/licence/check', {
      tornApiKey: settings.apiKey
    });

    updateLicencePanel(data);

    if (showToastOnSuccess) {
      showToast(
        data.licence?.active
          ? `Faction licence active: ${data.licence.daysRemaining || 0} days remaining.`
          : 'Faction proved, but licence is missing or expired.',
        data.licence?.active ? 'ok' : 'warn'
      );
    }
  ...
  async function requireActiveFactionLicence() {
    if (!settings.apiKey) {
      throw new Error('Save your limited Torn API key first so the script can prove your faction.');
    }
    ...
    if (!data.licence?.active) {
      throw new Error('Your faction licence is missing or expired. A faction API member must buy/extend it.');
    }

    return data;
  }

  async function buyExtendFactionLicenceFromPanel() {
    const key = getSavedTornApiKeyFromLicencePanel();

    if (!key || key.length < 8) {
      throw new Error('Paste your Torn API key first. Buying/extending requires faction API permission.');
    }

    settings.apiKey = key;
    saveSettings();

    const factionId = String($('licenceFactionId')?.value || settings.factionId || '').replace(/[^\d]/g, '');
    const xanaxCount = Math.max(1, Math.floor(Number(String($('licenceXanaxCount')?.value || '1').replace(/[^\d]/g, '') || 1)));
    const buyer = parseUserDisplay(settings.userDisplay || $('user')?.value || '');

    if (!factionId) {
      throw new Error('Check your faction first so the faction ID can be filled.');
    }

    const data = await backendRequest('/api/licence/extend', {
      tornApiKey: settings.apiKey,
      factionId,
      xanaxCount,
      buyer: {
        display: buyer.display || settings.userDisplay || '',
        name: buyer.name || '',
        id: buyer.id || ''
      }
    });

    settings.pendingLicenceOrder = data.order || null;
    updateLicencePanel(data);

    const p = data.payment || {};
    const order = data.order || {};
    const msg = p.note || `Send ${xanaxCount} Xanax to the script owner.`;
    const paymentBox = $('licencePaymentStatus');
    if (paymentBox) {
      paymentBox.className = `${APP}-preview ok`;
      const tier = p.licenceTier || order.licenceTier || {};
      paymentBox.innerHTML = `
        <b>RWPH-style payment order created.</b><br>
        Order: ${escapeHtml(order.id || '')}<br>
        Members: ${escapeHtml(String(p.memberCount ?? order.memberCount ?? 0))}<br>
        Current extend rate: 1 Xanax = ${escapeHtml(String(p.daysPerXanax || order.daysPerXanax || '?'))} day(s) ${tier.label ? `(${escapeHtml(tier.label)})` : ''}<br>
        Pending licence days: ${escapeHtml(String(p.pendingDays || order.addedDays || 0))}<br>
        ${escapeHtml(msg)}<br>
        <span class="${APP}-tiny">The licence is not extended until the order is applied after payment.</span>
      `;
    }

    saveSettings();

    if (p.paymentUrl || order.paymentUrl) {
      const url = p.paymentUrl || order.paymentUrl;
      setTimeout(() => {
        window.location.href = url;
      }, 700);
    }

    showToast('Payment order created. Opening Torn Xanax item page...', 'ok');

    return data;
  }

  async function applyPendingLicenceOrderFromPanel() {
    const orderId = cleanText($('licenceOrderId')?.value || settings.pendingLicenceOrder?.id || '');

    if (!orderId) {
      throw new Error('No pending licence order ID found.');
    }

    const key = getSavedTornApiKeyFromLicencePanel();
    if (!key || key.length < 8) {
      throw new Error('Paste your Torn API key first. Applying requires faction API permission.');
    }
    ...
    const data = await backendRequest(`/api/licence/orders/${encodeURIComponent(orderId)}/apply`, {
      tornApiKey: settings.apiKey,
      buyer: {
        display: buyer.display || settings.userDisplay || '',
        name: buyer.name || '',
    ...
    if (paymentBox) {
      paymentBox.className = `${APP}-preview ok`;
      paymentBox.innerHTML = `
        <b>Licence order applied.</b><br>
        Added ${escapeHtml(String(p.addedDays || data.licence?.lastAddedDays || 0))} day(s).<br>
        New expiry: ${data.licence?.expiresAt ? escapeHtml(new Date(Number(data.licence.expiresAt)).toLocaleString()) : 'Updated'}
      `;
    }

    saveSettings();
    showToast(`Licence extended by ${p.addedDays || data.licence?.lastAddedDays || 0} days.`, 'ok');
    return data;
  }

  async function createRequestViaBackend(user, amount, balance, requestRecordBase) {
    const data = await backendRequest('/api/requests', {
      requestId: requestRecordBase.id,
      user: {
      ...
      balance,
      discordName: normalizeDiscordName(settings.discordName),
      createdAt: requestRecordBase.createdAt,
      expiresAt: requestRecordBase.expiresAt,
      requesterStatusSyncBase: 'https://www.torn.com/factions.php?step=your',
      tornApiKey: settings.apiKey
    });

    return {
      ...requestRecordBase,
      id: data.request?.id || data.requestId || requestRecordBase.id,
      discordMessageId: data.request?.discordMessageId || data.discordMessageId || '',
      backendUrl: getActiveBackendUrl(),
      status: data.request?.status || data.status || 'pending',
      expiresAt: Number(data.request?.expiresAt || data.expiresAt || requestRecordBase.expiresAt)
    };
  }

  async function saveBackendWebhookSettingsFromPanel() {
    settings.backendUrl = getActiveBackendUrl();

    const requestWebhookUrl = cleanText($('webhookUrl')?.value || '');
    const userNotifyWebhookUrl = cleanText($('userNotifyWebhookUrl')?.value || '');

    saveSettings();

    if (!backendLooksConfigured()) throw new Error('Backend URL is not configured in this userscript.');
    if (!settings.apiKey) throw new Error('A Torn API key with faction access is required.');
    if (!webhookLooksValid(requestWebhookUrl)) throw new Error('Add a valid banker request webhook URL first.');
    if (!webhookLooksValid(userNotifyWebhookUrl)) throw new Error('Add a valid user notice webhook URL first.');

    await backendRequest('/api/admin/settings', {
      tornApiKey: settings.apiKey,
      requestWebhookUrl,
      userNotifyWebhookUrl
    });
    ...
    if ($('userNotifyWebhookUrl')) $('userNotifyWebhookUrl').value = '';

    showToast('Webhook URLs saved to backend using Torn faction API permission.', 'ok');
  }

  async function deleteBackendWebhookSettingsFromPanel() {
    settings.backendUrl = getActiveBackendUrl();
    saveSettings();
    ...
    await backendRequest('/api/admin/settings/delete', {
      tornApiKey: settings.apiKey
    });
    ...
    showToast('Webhook URLs deleted from backend.', 'ok');
  }

  async function getBackendRequestStatus(req) {
    if (!req?.id) return null;
    const data = await backendRequest(`/api/requests/${encodeURIComponent(req.id)}/status`, null, { method: 'GET' });
    return data.request || data;
  }

  async function fulfillRequestViaBackend(record, banker) {
    if (!settings.apiKey) throw new Error('A Torn API key with faction access is required.');
    return backendRequest(`/api/admin/requests/${encodeURIComponent(record.id)}/fulfill`, {
      tornApiKey: settings.apiKey,
      banker
    });
  }

  async function cancelRequestViaBackend(record, banker, reason = 'Unavailable vault funds') {
    if (!settings.apiKey) throw new Error('A Torn API key with faction access is required.');
    return backendRequest(`/api/admin/requests/${encodeURIComponent(record.id)}/cancel`, {
      tornApiKey: settings.apiKey,
      banker,
      reason
    });
  ...
  function saveRequestFromPanel() {
    ...
    settings.backendUrl = getActiveBackendUrl();
    if ($('limitedApiKey')) {
      const keyInput = cleanText($('limitedApiKey')?.value || '');
      if (keyInput && keyInput !== '********') settings.apiKey = keyInput;
    }
  ...
  async function makeRequest() {
    ...
    if (!backendLooksConfigured()) {
      showToast('Backend URL is not configured. Replace DEFAULT_BACKEND_URL before sharing this script.', 'warn');
      return;
    }

    if (!settings.apiKey) {
      showToast('Save your limited Torn API key first to prove your faction.', 'warn');
      return;
    }
    ...
    try {
      await requireActiveFactionLicence();

      const balance = {
        amount: null,
        formatted: 'Backend banker check required',
        source: 'backend-only',
        checkedAt: 0
      };
      ...
      const storedRecord = await createRequestViaBackend(user, amount, balance, requestRecordBase);

      storePendingRequest(storedRecord);

      addRequestNotification(
        'sent',
        `Your ${amount.formatted} vault request was sent to the backend for banker review. It expires in 5 hours.`,
        storedRecord.id
      );

      showToast(`Request sent through backend: ${amount.formatted}. It expires in 5 hours.`, 'ok');
      ...
      showToast(err.message || 'Backend request failed.', 'bad');
    } finally {
      setTimeout(() => {
        submitLocked = false;
        const btn2 = $('make');
  ...
  function findPendingRequestForControls() {
    ...
    if (params.requestId) {
      ...
      if (byId) {
        ...
        if (params.backendUrl && !byId.backendUrl) {
          byId.backendUrl = params.backendUrl;
        }
        return byId;
      }
    }
    ...
    if (params.userId && params.amountRaw) {
      ...
      if (local) {
        ...
        if (params.backendUrl && !local.backendUrl) {
          local.backendUrl = params.backendUrl;
        }
        return local;
      }
    }
    ...
    return {
      ...
      backendUrl: params.backendUrl || getActiveBackendUrl()
    };
  }
  ...
  async function cancelCurrentRequestUnavailableFunds(record, banker) {
    if (hasRequestAction(record.id, 'cancelledUnavailableFunds')) {
      removePendingRequest(record.id);
      throw new Error('This request was already canceled from this browser.');
    }
    ...
    await cancelRequestViaBackend(cancelledRecord, banker, cancelledRecord.cancelReason);

    markRequestAction(record.id, 'cancelledUnavailableFunds', { backendCancelled: true });
  ...
  async function markCurrentRequestFulfilled(record, banker) {
    if (hasRequestAction(record.id, 'fulfilled')) {
      removePendingRequest(record.id);
      throw new Error('This request was already marked fulfilled from this browser.');
    }
    ...
    await fulfillRequestViaBackend(completedRecord, banker);

    markRequestAction(record.id, 'fulfilled', { backendFulfilled: true });
  ...
  function openRequestPanel() {
    ...
    $('checkLicence').addEventListener('click', async () => {
      ...
      try {
        await saveAndCheckLimitedApiKey(true);
      } catch (err) {
        console.error('[Vault Request] Licence check failed:', err);
        showToast(err.message || 'Licence check failed.', 'bad');
      } finally {
    ...
    $('buyExtendLicence').addEventListener('click', async () => {
      const btn = $('buyExtendLicence');
      if (btn) {
        btn.disabled = true;
      ...
      try {
        await buyExtendFactionLicenceFromPanel();
      } catch (err) {
        console.error('[Vault Request] Buy/extend licence failed:', err);
        showToast(err.message || 'Buy/extend licence failed.', 'bad');
      } finally {
        const liveBtn = $('buyExtendLicence');
        if (liveBtn) {
          liveBtn.disabled = false;
          liveBtn.textContent = 'Buy Licence / Extend Licence';
        }
      }
    });

    $('applyLicenceOrder').addEventListener('click', async () => {
      const btn = $('applyLicenceOrder');
      if (btn) {
        btn.disabled = true;
      ...
      try {
        await applyPendingLicenceOrderFromPanel();
      } catch (err) {
        console.error('[Vault Request] Apply licence order failed:', err);
        showToast(err.message || 'Apply licence order failed.', 'bad');
      } finally {
        const liveBtn = $('applyLicenceOrder');
        if (liveBtn) {
          liveBtn.disabled = false;
          liveBtn.textContent = 'Apply Paid Order';
        }
      }
    });
  ...
  function openSettingsGate() {
    ...
    panel.innerHTML = `
      ...
      <div class="${APP}-card">
        ...
        <input id="${APP}-apiKeyGate" type="password" placeholder="Paste Torn API key with faction access" autocomplete="off" value="${settings.apiKey ? '********' : ''}" />

        <p class="${APP}-note">
          Settings control backend webhook setup, embed messages, banker balance checks, and protected backend actions.
      ...
      <div class="${APP}-card">
        ...
        <p class="${APP}-note">
          Your API key is saved only in this browser's local storage. Limited keys can prove faction for normal use. Faction-permission keys are required for settings and faction licence buy/extend.
        </p>
      </div>
    `;
  ...
  function updateEmbedPreview() {
    ...
    preview.innerHTML = `
      ...
      <div class="${APP}-tiny" style="margin-top:6px;">Buttons: Open Faction Controls + Open Player Profile. Discord posting is done by the backend.</div>
    `;
  }
  ...
  async function sendTestEmbed() {
    ...
    if (!backendLooksConfigured()) {
      showToast('Backend URL is not configured in the userscript.', 'warn');
      return;
    }

    if (!settings.apiKey) {
      showToast('A Torn API key with faction access is required.', 'warn');
      return;
    }
    ...
    try {
      await backendRequest('/api/admin/test', {
        tornApiKey: settings.apiKey,
        user,
        amount: {
          raw: amount.raw,
      ...
      showToast('Backend test embed sent to Discord.', 'ok');
    } catch (err) {
      console.error('[Torn Vault Request Maker] Backend test failed:', err);
      showToast(err.message || 'Backend test failed.', 'bad');
    }
  }
  ...
  function openSettingsPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="${APP}-card">
        ...
        <div class="${APP}-row">
          <button type="button" class="${APP}-btn good" id="${APP}-saveBackendWebhooks">Save Webhooks To Backend</button>
          <button type="button" class="${APP}-btn danger" id="${APP}-deleteBackendWebhooks">Delete Webhooks From Backend</button>
        </div>
    ...
    $('saveBackendWebhooks').addEventListener('click', async () => {
      const btn = $('saveBackendWebhooks');
      if (btn) {
        btn.disabled = true;
      ...
      try {
        await saveBackendWebhookSettingsFromPanel();
      } catch (err) {
        console.error('[Vault Request] Backend webhook save failed:', err);
        showToast(err.message || 'Could not save webhooks to backend.', 'bad');
      } finally {
        const liveBtn = $('saveBackendWebhooks');
        if (liveBtn) {
          liveBtn.disabled = false;
          liveBtn.textContent = 'Save Webhooks To Backend';
        }
      }
    });

    $('deleteBackendWebhooks').addEventListener('click', async () => {
      const btn = $('deleteBackendWebhooks');
      if (btn) {
        btn.disabled = true;
      ...
      try {
        await deleteBackendWebhookSettingsFromPanel();
      } catch (err) {
        console.error('[Vault Request] Backend webhook delete failed:', err);
        showToast(err.message || 'Could not delete webhooks from backend.', 'bad');
      } finally {
        const liveBtn = $('deleteBackendWebhooks');
        if (liveBtn) {
          liveBtn.disabled = false;
          liveBtn.textContent = 'Delete Webhooks From Backend';
        }
      }
    });
